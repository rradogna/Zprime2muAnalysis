    ...
    def prepare_histos_sum(self, samples, lumi):
        ...
        for nminus1 in nminus1s + ['NoNo']:
            ...
            for sample in samples:
                f = ROOT.TFile('./zp2mu_histos.root')
                h = f.Get(nminus1).Get('DileptonMass').Clone()
                print nminus1, sample
                h.Scale(1)
                hs.append(h)
            hsum = hs[0].Clone()
            for h in hs[1:]:
...
mcsum.prepare_histos_sum('./zp2mu_histos.root', lumi)
mc_samples = ['./zp2mu_histos.root']
...
to_use = {
    '60m120':  [mcsum], #, zmumu, ttbar],
    '120m200': [mcsum], #, dy120_c1, ttbar],
    '200m400': [mcsum], #, dy200_c1, ttbar],
    '400m600': [mcsum], #, dy200_c1, ttbar],
    '600m':    [mcsum], #, dy500_c1, ttbar],
    }
